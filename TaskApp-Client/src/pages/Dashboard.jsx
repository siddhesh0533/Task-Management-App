...
const StatCard = ({ label, value, color, delay }) => (
  <div className={`panel p-4 sm:p-5 animate-fade-up ${delay}`}>
    <p className="text-xs text-surface-400 uppercase tracking-widest font-medium mb-2">{label}</p>
    <p className={`text-2xl sm:text-3xl font-display font-bold ${color}`}
...
const Dashboard = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
      ...
      <div className="panel animate-fade-up stagger-2">
        ...
        {loading ? (
          ...
          <div className="text-center py-14 text-surface-500 text-sm">
            ...
            {user?.role === 'manager' && (
              <Link to="/tasks"
                className="text-accent-400 text-sm hover:underline mt-1 block">
            ...
            {recentTasks.map((task) => (
              <Link
                ...
                className="flex items-center justify-between px-4 sm:px-5 py-3.5
                           ...
                           hover:bg-surface-800/60 transition-colors group"
              >
      ...
      {/* Quick actions */}
      <div className="mt-4 sm:mt-5 flex gap-3 animate-fade-up stagger-3">
        {user?.role === 'manager' && (
